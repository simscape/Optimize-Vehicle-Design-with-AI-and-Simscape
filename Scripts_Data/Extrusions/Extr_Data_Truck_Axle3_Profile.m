function [xy_data_top, xy_data_side] = Extr_Data_Truck_Axle3_Profile(whl_rad, whl_base, body_height, thk, varargin)
%Extr_Data_Truck_Axle3_Profile Produce extrusion data for a semi-truck profile.
%   [xy_data_top, xy_data_side] = Extr_Data_Truck_Axle3_Profile(whl_rad, whl_base, thk, varargin)
...
%   >> Extr_Data_Truck_Axle3_Profile
%
%   To see a plot created with your parameter values,
%   add 'plot' as the final argument
%   >> Extr_Data_Truck_Axle3_Profile(0.43, 3.2, 0.05,'plot')
...
xy_grill = [0.7095+0.6 -0.2129];
xy_ell_hood = Extr_Data_Ellipse(1.2949,0.3548,180,360-45,1).*[-1 -1]+[-0.5854+0.6 1.2129*body_height/body_height_nominal+wrad_dl];
...
xy_ell_roof = [xy_ell_hood(end,:)+[-0.02 0.01];xy_ell_hood(end,:)+[-0.04 0.03];-0.7 3.0*body_height/body_height_nominal;-3.3 3.0*body_height/body_height_nominal];
xy_trunk = [...
    -3.3       0.6670+wrad_dl;...
    -3.8798-wbase_dl-wrad_dl    0.6670+wrad_dl];
...
xy_ell_rear3 = [-3.8798-wbase_dl-wrad_dl    0.0497+wrad_dl];
xy_ell_rear4 = [-3.8798-wbase_dl-wrad_dl   -0.2129];
...
car_top_profile  = [xy_grill;xy_ell_hood;xy_ell_roof;xy_trunk;xy_ell_rear3;xy_ell_rear4];
